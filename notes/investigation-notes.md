# Investigation Notes — Lab 65 Unquoted Service Path Investigation

## Investigation Overview

The investigation focused on identifying genuine unquoted service path conditions while avoiding false positives caused by service command-line arguments.

The initial detection logic produced a large result set, which became the main analytical finding of the lab.
...
1. The initial unquoted-path search returned 251 candidates.
2. The detection logic generated significant false positives.
3. Standard `svchost.exe` parameters caused many matches.
4. `WpnService` was running as `LocalSystem` with automatic startup.
5. `PrintWorkflowUserSvc_107314` used `svchost.exe -k PrintWorkflow`.
6. The `-k PrintWorkflow` string was correctly interpreted as a service argument.
7. File validation failed when the full command line was treated as a file path.
8. Event ID 7045 was available.
9. PowerShell Event ID 4104 was available for investigation commands.
10. Sysmon Event ID 3 was available.
11. No direct execution or exploitation evidence was established.

## Analyst Assessment

The investigation did not establish a confirmed exploitable unquoted service path.

The main finding was a **detection-logic weakness** that produced false positives.

The lab demonstrated why service command-line parsing is essential before classification.
...
The executable portion must first be isolated and evaluated independently of command-line parameters.
